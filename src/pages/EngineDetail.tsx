...
const EngineDetail = ({ model }: EngineDetailProps) => {
  const engineData: Record<string, any> = {
    ...
    "rs185": {
      name: "RS185",
      tagline: "Компактный двигатель среднего класса тяги",
      description: "Универсальный двигатель для повседневных полетов с отличным балансом мощности и экономичности. Идеально подходит для обучения и регулярных полетов.",
      specifications: {
        "Сухая масса": "Уточняется по паспорту партии",
        "Диаметр пропеллера": "Уточняется по паспорту партии",
        "Ориентировочная тяга": "Уточняется по паспорту партии",
        "Рекомендуемая смесь": "1:50 (бензин АИ-95 + масло 2T)",
        "Свеча зажигания": "NGK (уточняется по паспорту)",
        "Момент затяжки свечи": "25-30 Н·м"
      },
      features: [
        "Низкий уровень вибраций для комфортного полета",
        "Увеличенный моторесурс до 300 часов",
        "Совместимость с различными пропеллерами",
        "Экономичный расход топлива",
      ...
      compatibility: [
        ...
        "Рамы из нержавеющей стали",
        "Титановые рамы",
        "Стандартное крепление H&E",
        "Совместимость с ручным стартером",
        "Возможность установки электростартера"
      ],
      maintenance: {
        schedule: "Регламентное ТО каждые 25 моточасов или 6 месяцев",
        warranty: "Гарантия 12 месяцев с даты продажи",
